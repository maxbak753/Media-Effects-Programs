...
% Read Image  - - - - - - - - - - - - - - - - - 
%im = imread(strcat(path, vid_name));   % 0 - 255 (original)
...
if (im_vid == 1)
    ...
    frame_amount = 1000; % same as final # of clusters
    step = 3;
    ...
    new_vid_name = strcat(vid_name, '__VIDEO__', num2str(k),'_clstrs', '_L', num2str(q), '_loc', num2str(loc_use), '_', num2str(loc_importance_frac), '_clstr', num2str(frame_amount), '_step', num2str(step));
    ...
    while hasFrame(v_reader)
        ...
        frame_new = kMeans_Image(frame, k, components_use, loc_use, loc_importance_frac, wrt_toggle, plt_toggle, max_iter);
